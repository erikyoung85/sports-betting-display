...
export class LoopComponent implements AfterViewInit {
  @ContentChildren('loopItem', { read: TemplateRef }) items!: QueryList<
    TemplateRef<any>
  >;
  ...
  private initializeScroll() {
    const firstItem = this.items.first.elementRef.nativeElement;
    this.setScrollPos(
      Math.round(
        firstItem.getBoundingClientRect().left +
          this.getScrollPos() -
          (this.context.offsetWidth - firstItem.offsetWidth) / 2
      )
    );
  }
  ...
  private getClonesWidth(): number {
    let width = 0;
    this.items.forEach(
      (item) => (width += item.elementRef.nativeElement.offsetWidth)
    );
    return width;
  }
